...
代码截图：
https://github.com/crazy-luke/code2img/tree/main

轻量、上手简单
• 纯 JavaScript/TypeScript 调用，无需额外安装或配置 WebDriver；
• 内置常用功能（页面导航、渲染、截图、PDF 导出），直接调用即可完成大多数需求。
强大的截屏与页面操作能力
• 支持精准裁剪（clip）、视窗控制（viewport、deviceScaleFactor）；
• 可以执行自定义 JS 脚本，在页面里动态测量、注入样式、获取元素尺寸等；
• 支持无头与有头（headful）两种模式，调试时可显式看到浏览器界面。
